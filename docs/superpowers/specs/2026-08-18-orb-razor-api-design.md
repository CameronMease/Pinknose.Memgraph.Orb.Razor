...
| Risk | Mitigation |
|---|---|
| `mouse-move` may not populate `subject` for edges as reliably as for nodes | Spike first; fall back to subscribing `node-hover`/`edge-hover` directly |
| Complex types as `[JSInvokable]` **parameters** may not deserialize cleanly under WASM trimming | Spike first; fall back to passing a JSON string and deserializing manually |
