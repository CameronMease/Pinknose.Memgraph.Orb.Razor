...
| **Partially settled (Task 1 spike):** complex types as `[JSInvokable]` **parameters** bind cleanly under normal reflection-based deserialization. Verified against the running sample host — Blazor **Server**, `AddInteractiveServerComponents()` only, run via plain `dotnet run` (never published, never trimmed): a `Probe { Id, LocalX }` parameter on `[JSInvokable] Accept(string type, Probe payload)`, invoked from JS with camelCase `{ id: 'n1', localX: 12.5 }`, produced `payload.Id == "n1"` and `payload.LocalX == 12.5` with no errors — Blazor's interop deserializer matches camelCase JS properties to PascalCase C# properties automatically. **The original risk — that IL trimming during a WASM publish removes property metadata a reflection-based deserializer needs — was not exercised.** A Server circuit never goes through the trimmer; this spike could not and did not test that path, and it remains unverified. | **Chosen approach:** `OrbEventRelay.HandleOrbEvent(string type, OrbEventPayload payload)` proceeds as designed in §5.1, with a typed `OrbEventPayload` parameter — confirmed sufficient for the Server path. The JSON-string-plus-`OrbJsonContext` fallback is not needed for Server, but remains the designated fallback if a trimmed WASM publish later proves typed parameter binding breaks; that trimming risk stays open until a published-and-trimmed WASM sample exercises it. |
